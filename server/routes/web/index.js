module.exports = app => {
  ...
  router.get('/news/list', async (req, res) => {
    const parent = await Category.findOne({ // 查找文章分类
      name: '文章'
    })
    const cats = await Category.aggregate([
      { $match: { parent: parent._id } }, // 查找文章分类下的分类
      {
        $lookup: { // 查找该分类下的文章列表
          from: 'articles',
          localField: '_id',
      ...
      {
        $addFields: { // 获取该分类下5篇文章内容
          newsList: { $slice: ['$newsList', 5] }
        }
      }
    ])
    const subCats = cats.map(v => v._id) // 获取文章分类下的分类id
    cats.unshift({ // 取最新的5条文章作为热门文章
      name: '热门',
  ...
  router.get('/heroes/:id', async (req, res) => {
    const data = await Hero
      ...
      .populate('categories items1 items2 partners')
      .lean()
    for (let i = 0; i < data.partners.length; i++) {
      const partner = await Hero
        .findById(data.partners[i].hero)
        .lean();
      if (partner) {
        data.partners[i].avatar = partner.avatar;
      } else {
        data.partners[i].avatar = '';
      }
    }
    res.send(data)
  })
  ...
  router.post('/tea', async (req, res) => {
    ...
    try {
      const model = await Tea.create(req.body);
    } catch (e) {
      console.error(e);
    }
    res.send({
